...
| 文件 | 策略组 | 规则提供商 | 说明 |
| --- | ---: | ---: | --- |
| [`rules/DustinWin-full.yaml`](./rules/DustinWin-full.yaml) | 17 | 18 | DustinWin 规则集完整版：完整 Google 优先进入 `节点选择`，避免全球 `.cn` 服务误直连；保留 AI、流媒体、游戏平台、Telegram、Apple、Microsoft、OneDrive、地区节点及内置 DNS/嗅探 |
| [`rules/DustinWin-core.yaml`](./rules/DustinWin-core.yaml) | 4 | 10 | DustinWin 规则集核心白名单版：完整 Google 优先进入 `节点选择`；完整 Apple / Microsoft 与国内白名单进入 `全球直连`，保留 DNS、嗅探和状态持久化 |
| [`rules/DustinWin-nano.yaml`](./rules/DustinWin-nano.yaml) | 5 | 6 | DustinWin 规则集极简版：完整 Google 与明确非中国域名优先代理，国内直连和兜底分流；不接管 DNS |
...
- **Full 场景分流**：完整模板保留 AI、流媒体、游戏平台、Telegram、Apple、Microsoft、OneDrive 等独立入口。DustinWin Full 用完整 Google 规则优先进入 `节点选择`；Geodata Full 中游戏规则优先级高于 `category-entertainment`，避免游戏域名被娱乐/流媒体大类提前接走；GitHub 先于 Microsoft 单独进入 `节点选择`，避免被官方 Microsoft 大类带入直连。
- **Core 白名单分流**：核心模板不保留服务 UI 分组和地区节点组；DustinWin Core 的完整 Google 规则优先进入 `节点选择`，完整 Apple / Microsoft 规则和国内白名单进入 `全球直连`，该组默认 `DIRECT`、可切到 `节点选择` 或 `自动测速`，其余流量全部交给 `MATCH,节点选择`。ACL4SSR Core 在 `ChinaDomain` 前保留 `ProxyLite`；MetaCubeX Core 在 `GEOSITE,cn` 前保留 `GEOSITE,geolocation-!cn`，防止 `googleapis.cn` 等上游明确非中国域名被宽泛的 `.cn` 分类提前送去直连。
- **Nano 极简代理**：DustinWin Nano 使用完整 Google + `proxy`，Geodata Nano 使用 `geolocation-!cn`，ACL4SSR Nano 使用 `ProxyLite`，代理明确非中国域名；三者都保留国内直连和兜底，不提供地区节点或服务分组。
...
| 4 | 国内游戏 | `全球直连`（默认 `DIRECT`，可切 `节点选择` / `自动测速`） |
...
- **DustinWin-full**：`private` · `privateip` · `google`（blackmatrix7，完整 Google 规则，进入 `节点选择`）· `trackerslist`（BT Tracker）· `apple-cn` · `apple`（blackmatrix7）· `microsoft-cn` · `microsoft`（blackmatrix7）· `onedrive`（blackmatrix7）· `games-cn` · `ai` · `mediaip` · `games` · `telegramip` · `cn-lite`（路由直连）· `cn`（DNS 国内解析）· `cnip` · `fakeip-filter`（仅供 `dns.fake-ip-filter`）
- **DustinWin-core**：`private` · `privateip` · `google`（blackmatrix7，完整 Google 规则，进入 `节点选择`）· `apple`（blackmatrix7，完整 Apple 规则，仅用于路由）· `microsoft`（blackmatrix7，完整 Microsoft 规则，仅用于路由）· `games-cn` · `cn-lite`（路由直连）· `cn`（DNS 国内解析）· `cnip` · `fakeip-filter`（仅供 `dns.fake-ip-filter`）
- **DustinWin-nano**：`private` · `privateip` · `google`（blackmatrix7，完整 Google 规则，进入 `节点选择`）· `proxy` · `cn-lite`（路由直连）· `cnip`
