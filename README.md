...
| [`Core.yaml`](./Core.yaml) | 9 | 10 | 核心白名单版：保留基础节点选择、国内直连和 DNS；完整 Apple / Microsoft 直接进入 `全球直连`，其余未命中流量统一进入 `节点选择` |
...
- **Core 白名单分流**：核心模板不保留服务 UI 分组；仅将完整 Apple / Microsoft 规则和国内白名单直连，其余流量全部交给 `MATCH,节点选择`。
