...
- 局域网 / 私有地址：`DIRECT`
- 中国大陆相关域名与 IP：`DIRECT`
- 游戏平台：中国区服务和 Steam 下载区域检测直连，下载 CDN 和平台流量进入 `游戏平台`
- 海外 AI 服务：统一进入 `AI出口`
- 流媒体 / 娱乐服务：统一进入 `流媒体`
...
这样可以避免社交通讯等服务各自拆成小组导致的选择负担，同时只为 AI、流媒体、Google、Apple、Microsoft、开发工具、游戏平台保留少量确实有独立出口价值的大类入口。规则层使用 ACL4SSR 的 `rule-providers`，减少对客户端内置 `GeoSite.dat` 分类名称的依赖。GitHub 和开发工具规则放在流媒体与游戏平台规则之后、Google / Apple / Microsoft 规则之前，用于控制代码托管、包管理器、开发文档等开发相关流量的出口地区。
...
- 本配置使用 ACL4SSR 的远程 `rule-providers`，首次加载和定期更新需要客户端能访问对应规则 URL；国内 IPv4 / IPv6 规则后仍保留 `GEOIP,CN` 作为兜底。
...
- 游戏相关流量保留 `游戏平台` 独立入口；`SteamCN` 和 Steam 下载区域检测直连，下载 CDN 和 Steam / Epic / Origin / Blizzard / Nintendo / Sony / Xbox 等平台规则进入 `游戏平台` 以保留手动切换能力。
...
- `开发工具` 使用 ACL4SSR 的 `Github` 和 `Developer` 规则，并放在 `Microsoft` 之前，避免 GitHub 等开发流量被微软服务或普通代理规则提前命中；二者都放在 `ProxyLite` 和国内兜底规则之前。
