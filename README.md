...
Core 的 `苹果服务`、`微软服务` 与 `直连` 默认选择直连；`OneDrive` 域由 `微软服务` 承接（在 geosite microsoft 内），`GitHub` 由独立规则（纯 domain）导向 `节点选择`（均无独立组），可在面板中切换。Core/Nano 不提供 `手动切换`，所有模板的 `漏网之鱼` 默认选择 `节点选择`。
