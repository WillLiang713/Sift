...
| GitHub | Dedicated rule (`geosite/github`, pure domain) → `节点选择`, placed before `microsoft`; Nano → `节点选择`/`漏网之鱼` |
...
- Core routes Apple and Microsoft rules to separately controllable service groups, both defaulting to `直连`; `GitHub` has a dedicated rule (`geosite/github` → `节点选择`, before `microsoft` since geosite microsoft includes github) but no group; `OneDrive` has no rule — onedrive domains are covered by `geosite microsoft` → `微软服务`.
