...
| GitHub | No dedicated rule/group: github domains fall to `proxy` → `节点选择`; Nano → `节点选择`/`漏网之鱼` |
| OneDrive | No dedicated rule: domains covered by blackmatrix7 `Microsoft.list` → `微软服务` (default `直连`); Nano → `节点选择` |
...
- Core routes Apple and Microsoft rules to separately controllable service groups, both defaulting to `直连`; `GitHub`/`OneDrive` have no dedicated rules/groups — github domains fall through to `proxy` → `节点选择`; onedrive domains are covered by blackmatrix7 `Microsoft.list` → `微软服务`.
