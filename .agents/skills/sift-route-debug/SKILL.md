...
| --- | --- |
| **Google/Play anchors** | **`googleapis.cn`** + **`play.googleapis.com`**: Full/Core → `谷歌服务`; Nano → `节点选择`. Must not fall through to broad CN direct. DNS half of the same contract is Full `proxy → fake-ip` / Core `google` + `proxy → fake-ip`, after higher-priority unconditional-direct real-IP exceptions. |
...
| Other Google | `www.google.com` same policy split as anchors (Full/Core `谷歌服务` / Nano `节点选择`). `recaptcha.net`: Full/Core → `谷歌服务` via geosite `google` before `cn-lite`. |
| CF challenge | Full binds `challenges.cloudflare.com` to `流媒体` via the DustinWin `media` set. |
| Full/Core AI | `chatgpt.com` → `AI` |
| YouTube / streaming | Full YouTube and the DustinWin `media` set → `流媒体`; Core YouTube → `谷歌服务` (no streaming group); Nano → `节点选择` |
| Full brands | icloud → `苹果服务`, office → `微软服务` |
| Core brands | AI → `AI`、GitHub → `GitHub`、Google → `谷歌服务` (default `节点选择`); Apple → `苹果服务`, Microsoft → `微软服务` (default `全球直连`) |
...
| Domestic | baidu/qq/taobao/bilibili → `全球直连` on all three |
| Private | localhost → `DIRECT` |

Display-only (not FAIL/WARN):

- `gstatic.cn` may → `直连` through `cn-lite` on Nano; Core follows geosite `google` → `谷歌服务`. It is **not** a Play/API contract anchor.
...
- `proxy` is the explicit non-CN layer after service/brand rules and before `cn-lite`; Full and Core have `google` → `谷歌服务` before `proxy` / `cn-lite`.
...
- Core routes AI / GitHub / Google / Apple / Microsoft rules to separately controllable service groups (`AI` / `GitHub` / `谷歌服务` default to `节点选择`; Apple/Microsoft default to `全球直连`). geosite `google` is before `cn-lite` so recaptcha / google.cn are not swallowed by domestic direct. Full and Core both have a dedicated `GitHub` group (rule `geosite/github` before `microsoft` since geosite microsoft includes github); Nano routes github to `节点选择`/`漏网之鱼`. `OneDrive` has no rule — onedrive domains are covered by `geosite microsoft` → `微软服务`. No template routes an ads set.
