...
- `ruleset/` stores remotely cached rule-set files (DustinWin `.list` text providers plus the blackmatrix7 overseas Apple/Microsoft/OneDrive sets, all classical `.list`).
...
Remote rule sets come from [DustinWin/ruleset_geodata](https://github.com/DustinWin/ruleset_geodata) by default and are wired as `format: text` `.list` providers for compatibility; keep their `behavior` as `domain` or `ipcidr` according to the source set. The exceptions are the overseas Apple/Microsoft/OneDrive sets, taken from [blackmatrix7/ios_rule_script](https://github.com/blackmatrix7/ios_rule_script) because DustinWin publishes none. Those are wired as `classical`/`text` `.list` providers (`apple` ← `rule/Clash/Apple/Apple.list`, `microsoft` ← `rule/Clash/Microsoft/Microsoft.list`, `onedrive` ← `rule/Clash/OneDrive/OneDrive.list`). Microsoft and OneDrive *require* classical: their lists cover flagship domains (`microsoft.com`, `1drv.ms`, …) only via `DOMAIN-KEYWORD`, and domain-only formats cannot store keyword rules, so such builds would silently drop them. The classical lists also bring Apple's `IP-CIDR,…,no-resolve` ranges and OneDrive's `PROCESS-NAME` rules. All blackmatrix7 paths are free of `geosite`/`geoip`. Regardless of source, rule-provider URLs must avoid the substrings `geosite` and `geoip` anywhere in their paths. Rule order matters: the OneDrive rule must precede the Microsoft rule (OneDrive domains also live in Microsoft's list), and the Microsoft rule must sit after AI (its broad `microsoft` keyword would otherwise swallow `copilot.microsoft.com`). ShellCrash scans rule-provider URLs and treats those keywords as a signal that the Geo databases (`geoip.metadb` / `geosite.dat`) are required, which triggers extra Geo-data downloads and checks that slow down or disrupt startup. DustinWin's `.list` paths do not contain those keywords, so startup stays clean; MetaCubeX `meta-rules-dat` URLs (for example `.../geo/geosite/...` or `.../geoip/...`) do contain them and trigger the misdetection. Do not switch rule-provider URLs to MetaCubeX even when the rule content is equivalent — keep DustinWin, or another source whose URL path is free of those keywords. The `fakeip-filter` provider (DustinWin, `domain`/`text`) is the lone provider referenced only by `dns.fake-ip-filter` (not by any `rules` entry); domestic direct providers may be referenced by both `rules` and `dns.fake-ip-filter`. sift-check counts a `rule-set:` reference inside the `dns:` block as usage, so DNS-only providers are not flagged as orphans.
...
Keep YAML indentation at two spaces and group rules by routing intent, with short comments explaining each block. Preserve established strategy-group names such as `手动切换`, `自动测速`, `漏网之鱼`, `节点选择`, `AI`, `流媒体`, `游戏平台`, and `Telegram` unless a routing change requires renaming them. Keep `Nano.yaml` intentionally smaller than `Full.yaml`; do not add AI, entertainment, gaming, Telegram, or region node groups there unless the template goal is explicitly changed. Keep each `rule-providers` key synchronized with the upstream rule-set file basename, for example use `google-cn` for `google-cn.list` and `ai` for `ai.list` instead of display-style names; `cn` is the deliberate exception for route compatibility and maps to `cn-lite.list`, while Full's DNS-only `cn-dns` maps to the full `cn.list` domestic set for nameserver-policy / fake-ip-filter coverage. Do not replace the routing `cn` provider with full `cn.list`; the full set can over-direct domains that should fall through to proxy. For overlapping rules, place the more specific or higher-intent rule first. Top-level scenario `select` groups should keep a consistent candidate order and use `include-all: true` so each scenario can directly select a concrete node without adding one-off duplicate groups such as `开发自建节点`. The `其他节点` group is the deliberate complement of the region node groups (`香港节点`, `美国节点`, `日本节点`, `新加坡节点`): it uses `include-all: true` + `exclude-filter` (not a `filter` negative-lookahead), and its `exclude-filter` must stay the exact union of those region groups' `filter` keywords — emoji flags and the `(?i)` case-insensitive flag included. Whenever you add or remove a keyword in any region group's `filter`, sync `其他节点`'s `exclude-filter` in the same edit; otherwise the affected nodes get double-listed (matched node also leaks into `其他节点`) or, for a removed keyword, silently dropped from both. In JavaScript, use two-space indentation, semicolons, `const`/`let`, and small helper functions as shown in `scripts/convert.js`. Prefer descriptive argument aliases and avoid changing existing aliases unless backward compatibility is intentionally broken.
