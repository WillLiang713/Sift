...
- `rules/DustinWin-full.yaml` is the full node-free DustinWin-based template. It carries top-level Mihomo runtime controls (`ipv6: true`, `unified-delay: true`, `tcp-concurrent: true`) plus top-level `profile:` (persisting selected strategy groups and fake-ip mappings), `dns:` (fake-ip whitelist mode, `ipv6: true` for AAAA answers, DustinWin `proxy` as the fake-IP whitelist, `rule-set:proxy` forced to overseas DoH via `nameserver-policy` while `rule-set:cn,private` uses domestic DoH, `respect-rules`, `direct-nameserver` for other DIRECT traffic, and overseas default DoH), and `sniffer:` blocks. Keep both IPv6 settings in all Full/Core templates. It keeps the common `广告拦截` group, AI, streaming, gaming platform, Telegram, Apple, Microsoft, OneDrive, and region node strategy groups. DustinWin `ads` is placed immediately after private rules, while `proxy` (explicit non-CN domains) routes to `节点选择` after service/scene groups and before `cn-lite`; the same domain-only provider is the DNS fake-IP whitelist so global `.cn` exceptions such as `googleapis.cn` enter Mihomo before any China-IP bypass.
...
- `rules/MetaCubeX-*.yaml` stores the public GEOSITE/GEOIP routing variants. These templates use MetaCubeX `meta-rules-dat` and define `geox-url`; their routing `rules` must remain pure `GEOSITE`/`GEOIP` with no routing `RULE-SET` and no `rule-providers`. Full/Core DNS uses fake-IP whitelist mode with `geosite:geolocation-!cn` plus `geosite:google`; domestic DNS policy uses `geosite:cn` / `geosite:private`.
- `rules/ACL4SSR-*.yaml` stores the ACL4SSR Clash `.list` variants. Full/Core use fake-IP whitelist mode with a DNS-only DustinWin `proxy` domain provider because ACL4SSR `ProxyLite` is classical and unsuitable for DNS references; MetaCubeX `cn.mrs` + `private` remain nameserver-policy inputs. Routing still uses ACL4SSR `ChinaDomain`. Advertising uses upstream `UnBan` before `BanAD` / `BanProgramAD`; keep `ProxyLite` before `ChinaDomain`, because `ChinaDomain` contains broad `.cn` rules while `ProxyLite` classifies exceptions such as `googleapis.cn` for `节点选择`.
...
The `rules/MetaCubeX-*.yaml` templates intentionally use MetaCubeX `meta-rules-dat` via top-level `geox-url`. Keep routing rules as `GEOSITE,...` / `GEOIP,...` only and do not define `rule-providers`; Full/Core fake-IP whitelist and domestic DNS policy use geosite selectors directly. Public filenames are `rules/MetaCubeX-full.yaml`, `rules/MetaCubeX-core.yaml`, and `rules/MetaCubeX-nano.yaml`.
