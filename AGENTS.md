...
Remote rule sets come from [DustinWin/ruleset_geodata](https://github.com/DustinWin/ruleset_geodata) by default and are wired as `format: text` `.list` providers for compatibility; keep their `behavior` as `domain` or `ipcidr` according to the source set. Full/Core DNS also pulls MetaCubeX `geosite` `cn` as MRS (`format: mrs`) for domestic real-IP / nameserver-policy coverage.
...
The `rules/MetaCubeX-*.yaml` templates intentionally use MetaCubeX `meta-rules-dat` via top-level `geox-url`. Keep routing rules as `GEOSITE,...` / `GEOIP,...` only; the only permitted `rule-providers` entry is the DNS-only `fakeip-filter` provider in MetaCubeX Full/Core. Domestic DNS uses `geosite:cn` / `geosite:private`. Public filenames are `rules/MetaCubeX-full.yaml`, `rules/MetaCubeX-core.yaml`, and `rules/MetaCubeX-nano.yaml`.
