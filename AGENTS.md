...
- `rules/ACL4SSR-*.yaml` stores the ACL4SSR Clash `.list` variants. Full/Core align DNS rule-set providers with DustinWin templates by using DustinWin `fakeip-filter`, `private`, and `cn`; ACL4SSR classical lists stay routing-only.
...
- `rules/MetaCubeX-core.yaml` keeps the same 4-group Core contract and routes full `GEOSITE,apple` and full `GEOSITE,microsoft` to `全球直连`; `GEOSITE,github,节点选择` must stay immediately after private rules, and final fallback remains `MATCH,节点选择`. Do not add dedicated Google rules unless the MetaCubeX routing design changes.
