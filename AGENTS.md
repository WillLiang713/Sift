...
- `rules/ACL4SSR-*.yaml` stores the ACL4SSR Clash `.list` variants. Full/Core align DNS rule-set providers with DustinWin templates by using DustinWin `fakeip-filter`, `private`, and `cn`; ACL4SSR classical lists stay routing-only. All three use the repository-owned `google-play-cn` classical provider before Google / `ProxyLite`, routing v2fly `google-play@cn` Beijing (`2x3`), Shanghai (`ni5`), and Guangzhou (`j5o`) download nodes to `全球直连` while keeping Google control and overseas download domains proxied. Keep `ProxyLite` before `ChinaDomain` in all ACL4SSR variants that define both, because `ChinaDomain` contains the broad `DOMAIN-SUFFIX,cn` rule while `ProxyLite` explicitly classifies exceptions such as `googleapis.cn` for `节点选择`. ACL4SSR Full routes streaming via ACL brand packs (`YouTube`, `Netflix`, `NetflixIP`, `DisneyPlus`, `Spotify`, `TikTok`) to `流媒体` before `Google`/`ProxyLite`; do not re-add aggregate `ProxyMedia` (it pulls non-content hosts such as `challenges.cloudflare.com` into the media group).
- `rulesets/google-play-cn.list` is the repository-owned Mihomo classical conversion of the `@cn` regex entries from v2fly `data/google-play`; keep its single regional matcher synchronized with upstream and reference it before Google / `ProxyLite` in all ACL4SSR templates.
