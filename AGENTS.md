...
- `rules/DustinWin-full.yaml` is the full node-free DustinWin-based template. It carries top-level Mihomo runtime controls (`ipv6: true`, `unified-delay: true`, `tcp-concurrent: true`) plus top-level `profile:` (persisting selected strategy groups and fake-ip mappings), `dns:` (fake-ip, `ipv6: true` for AAAA answers, DustinWin `fakeip-filter` / `private` plus MetaCubeX `cn.mrs` for both `fake-ip-filter` and `nameserver-policy`, `respect-rules`, `direct-nameserver` for other DIRECT traffic, and overseas default DoH), and `sniffer:` blocks (HTTP/TLS/QUIC domain sniffing for TUN/redir-host accuracy). Keep both IPv6 settings in all Full/Core templates: the top-level switch allows IPv6 connections, while `dns.ipv6: true` returns AAAA answers. To disable IPv6 for leak hardening, set both to `false`. It keeps the common `广告拦截` group (default `REJECT`, temporarily switchable to `DIRECT` or `节点选择`), AI, streaming, gaming platform, Telegram, Apple, Microsoft, OneDrive, and region node strategy groups. DustinWin `ads` is placed immediately after private rules, while `proxy` (explicit non-CN domains) routes to `节点选择` after service/scene groups and before `cn-lite`, aligning with MetaCubeX `GEOSITE,geolocation-!cn` placement.
- `rules/DustinWin-core.yaml` is the core whitelist DustinWin-based template. It stays node-free, keeps the same runtime/profile/DNS/sniffer foundation as the full template, keeps only the base selector / `全球直连` groups plus the common `广告拦截` group, routes full Apple and full Microsoft rule sets plus domestic whitelist rules to `全球直连`, routes blackmatrix7 `onedrive` to `节点选择` immediately before `microsoft` (no OneDrive UI group; OneDrive domains also appear in Microsoft's broad list), routes DustinWin `proxy` to `节点选择` after those direct-service rules and before `cn-lite`, and Core's `全球直连` contains `DIRECT`, `节点选择`, and `自动测速` with `DIRECT` first; all other unmatched traffic falls through directly to `MATCH,节点选择`.
- `rules/DustinWin-nano.yaml` is the nano DustinWin-based template and should remain node-free and DNS-free; it keeps only `节点选择`, `手动切换`, `自动测速`, `全球直连`, `广告拦截`, and `漏网之鱼`, with DustinWin `ads` as the single advertising layer and `proxy` as the sole explicit non-CN proxy layer before domestic fallbacks (aligned with MetaCubeX `geolocation-!cn` / ACL4SSR `ProxyLite`).
- `rules/MetaCubeX-*.yaml` stores the public GEOSITE/GEOIP routing variants. These templates use MetaCubeX `meta-rules-dat` and define `geox-url`; their routing `rules` must remain pure `GEOSITE`/`GEOIP` with no routing `RULE-SET`. `rules/MetaCubeX-full.yaml` and `rules/MetaCubeX-core.yaml` may define the DNS-only `fakeip-filter` and `trackerslist` providers; domestic DNS uses `geosite:cn` / `geosite:private` (same MetaCubeX cn data as `cn.mrs`). MetaCubeX GeoSite has no `trackerslist` tag, so the separate provider is required.
- `rules/ACL4SSR-*.yaml` stores the ACL4SSR Clash `.list` variants. Full/Core align DNS with DustinWin: `fakeip-filter` / `private` plus MetaCubeX `cn.mrs` for real-IP and domestic `nameserver-policy`. Routing still uses ACL4SSR `ChinaDomain` (not the DNS `cn` provider). Advertising uses upstream `UnBan` before `BanAD` / `BanProgramAD` in all variants; both blocking lists route to the common `广告拦截` group. Keep `ProxyLite` before `ChinaDomain` in all ACL4SSR variants that define both, because `ChinaDomain` contains the broad `DOMAIN-SUFFIX,cn` rule while `ProxyLite` explicitly classifies exceptions such as `googleapis.cn` for `节点选择`. Domestic IP fallback uses `ChinaIp` / `ChinaIpV6` only; do not append `GEOIP,CN`. ACL4SSR Full routes streaming via ACL brand packs (`YouTube`, `Netflix`, `NetflixIP`, `DisneyPlus`, `Spotify`, `TikTok`) to `流媒体` before `Google`/`ProxyLite`; do not re-add aggregate `ProxyMedia` (it pulls non-content hosts such as `challenges.cloudflare.com` into the media group).
...
Remote rule sets come from [DustinWin/ruleset_geodata](https://github.com/DustinWin/ruleset_geodata) by default and are normally wired as `format: text` `.list` providers for compatibility; keep their `behavior` as `domain` or `ipcidr` according to the source set. Full/Core DNS also pulls MetaCubeX `geosite` `cn` as MRS (`format: mrs`) for domestic real-IP / nameserver-policy coverage and DustinWin `trackerslist.mrs` for Tracker real-IP only.

All Full/Core templates use DustinWin `trackerslist.mrs`, built from the domain-only portions of [XIU2/TrackersListCollection](https://github.com/XIU2/TrackersListCollection) and [ngosang/trackerslist](https://github.com/ngosang/trackerslist). Keep it `behavior: domain` / `format: mrs`, reference it only from `dns.fake-ip-filter`, and never add a routing `RULE-SET,trackerslist,...` rule: Tracker domains need real-IP but must continue through normal routing.
...
- `rules/DustinWin-nano.yaml`: uses DustinWin `private`, `privateip`, `ads`, `proxy`, `cn-lite`, and `cnip` only; `ads` precedes `proxy`, which remains the sole explicit non-CN proxy layer before `cn-lite`.
...
The `rules/MetaCubeX-*.yaml` templates intentionally use MetaCubeX `meta-rules-dat` via top-level `geox-url`. Keep routing rules as `GEOSITE,...` / `GEOIP,...` only; the only permitted `rule-providers` entries in MetaCubeX Full/Core are the DNS-only `fakeip-filter` and `trackerslist` providers. Domestic DNS uses `geosite:cn` / `geosite:private`. Public filenames are `rules/MetaCubeX-full.yaml`, `rules/MetaCubeX-core.yaml`, and `rules/MetaCubeX-nano.yaml`.

## Build, Test, and Development Commands

There is no package manager manifest and no generated build step. Use lightweight validation before committing:

- `bash .claude/skills/sift-check/check.sh` (or `/sift-check` in Claude Code) checks project invariants: strategy-group / rule-set referential integrity, node-free rules, DNS allowance per template, canonical group scopes, geodata routing purity plus the DNS-only `fakeip-filter` / `trackerslist` exceptions, Tracker real-IP-only behavior, and optional `mihomo` / `yamllint` validation when installed.
...
Keep each `rule-providers` key synchronized with the upstream rule-set file basename when practical. Use `cn-lite` for DustinWin **routing** domain fallback. Full/Core DNS uses DustinWin `fakeip-filter` + `private` (MetaCubeX: `geosite:private`) plus MetaCubeX geosite **cn** for **both** `fake-ip-filter` and `nameserver-policy` (DustinWin/ACL4SSR: `rule-set:cn` → `cn.mrs`; MetaCubeX: `geosite:cn`). It also uses DustinWin `trackerslist.mrs` only in `fake-ip-filter`; do not add it to `nameserver-policy` or routing. Do **not** put DNS MetaCubeX `cn` into DustinWin routing in place of `cn-lite`, and do **not** use Loyalsoldier `direct` or blackmatrix7 `ChinaMaxNoIP` for DNS (Play/GVT false positives). Broader non-matching DIRECT traffic still uses `respect-rules` + `direct-nameserver`. Deliberate exceptions:

- blackmatrix7 service keys (`google`, `apple`, `microsoft`, `onedrive`) map to capitalized upstream paths.
- DustinWin / ACL4SSR Full/Core define MetaCubeX `cn` and DustinWin `trackerslist` as DNS-only MRS providers (routing stays `cn-lite` / `ChinaDomain`; Tracker routing is unchanged).
