...
当前模板为 [core.yaml](../rules/core.yaml)（大陆白名单）、[gfwlist.yaml](../rules/gfwlist.yaml)（GFWlist）与 [full.yaml](../rules/full.yaml)（完整服务组）。三档共用 fake-IP 地址池、“代理域名走海外 DoH”的分工和映射持久化，区别在“谁需要代理”以及 Full 是否按服务拆组。
...
客户端应接管 DNS，使用 fake-ip，保留模板的规则过滤和地址池，不用客户端旧配置覆盖。Nikki 的大陆 IPv4/IPv6 旁路分别开启后，目标 IP 命中它自带的 `geoip_cn.nft` / `geoip6_cn.nft` 即提前放行、不进内核；该名单与 DustinWin `cnip` 同源，实测 v4 7328/7362、v6 1528/1655 条目完全一致，因此旁路集合约等于模板本就判直连的集合，走节点的 fake-IP 连接不会被抢走。`core.yaml` 与 `gfwlist.yaml` 的未收录域名拿的是 fake-IP，即使后续解析到大陆 IP 也是在内核中直连；`full.yaml` 的未收录域名是 real-IP，若真解析到大陆 IP 会被提前旁路，但它走海外 DoH 解析，实际很少触发。模板不会自动开启这些开关；验证 YAML 完整规则时应关闭旁路，让流量进入内核。IPv6 流量也需要接管；若客户端不支持，应同时关闭顶层 `ipv6` 和 `dns.ipv6`。
