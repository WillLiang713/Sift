...
Sift 不启用 fallback：`core.yaml` 的国内白名单走国内 DoH，`proxy` 与其余经代理走海外 DoH；`gfwlist.yaml` 只让 `gfw` 域名经代理走海外 DoH，其余走国内 DoH。大陆 IP 由 cnip 规则判断，不依赖 GeoIP 数据库。详见 [DNS 设计](./dns.md)。
